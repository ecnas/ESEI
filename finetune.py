...
def get_args():
    ...
    parser.add_argument('--total-epochs', type=int, default=50, help='total epochs')
    parser.add_argument('--learning-rate', type=float, default=1e-3, help='init learning rate')
    parser.add_argument('--save', type=str, default='finetune', help='dir for saving checkpoint')
    
    parser.add_argument('--code', type=list, default=[2, 0, 3, 0, 0, 3, 1, 1, 3, 3, 1, 1, 2, 1, 0, 3, 1, 2, 2], help='code for architecture')
    parser.add_argument('--model', type=str, default='checkpoints', help='dir for loading checkpoint')
    
    parser.add_argument('--img-dir', type=str, default='../', help='dir of images')
...
def train(model, device, loader, optimizer, code):
    _lambda = 1e-1
    model.train()
    train_cls_loss_all = []
    train_dice_all = []
...
def valid(model, device, loader, code):
        model.eval()
        valid_dice_all = []
        n_val = len(loader)  
        correct = 0
...
def main():
    ...
    resize_transforms = transforms.Resize([image_size[args.code[0]],]*2)
    ...
    logging.info("checkpoint {} loaded!".format(args.model))
    
    model.to(device)
    optimizer = optim.SGD(model.parameters(), lr=learning_rate) 

    loader = DataLoader(dataset, batch_size=batch_size, num_workers=8, pin_memory=True, shuffle=True)
    loader_val = DataLoader(dataset_val, batch_size=batch_size, num_workers=8, pin_memory=True, shuffle=False)
    
    valid_dice, acc, pred_prob_all, ground_truth = valid(model, device, loader_val, args.code[1:])
    se, sp, acc = calc_metrics(pred_prob_all, ground_truth)
    logging.info("#Before finetuning# dice:{}, se:{}, sp:{}, acc:{}".format(valid_dice, se, sp, acc))
    ...
    for epoch in range(n_epochs):
        ...
        train_dice = train(model, device, loader, optimizer, args.code[1:])
